...
/     ( args[2]->ip_daddr == ADDR || ADDR == NULL ) /
{
	...
	@a["max congestion window"] = max(args[3]->tcps_cwnd);
	@b["max send window"] = max(args[3]->tcps_swnd);
	@sendorcongestion[args[3]->tcps_swnd <= args[3]->tcps_cwnd ? "send window usage:" : "congestion window usage:"] = count(); 
}
tcp:::receive
